...
pygame.time.set_timer(obstacle_timer, 900) #Pygame internally starts a timer that pushes an event with type obstacle_timer onto the event queue every 1500 milliseconds.
...
while True:  #runs forever, unless broken from the inside
    for event in pygame.event.get(): #loops through every event that happened since it was last called
        ...
        if game_active == True: 
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                game_active = True
                snail_rect.bottomright = (600,300) #reset snail position
    ...
    if game_active:  #needed for creating different game states
        ...
        screen.fill((94, 129, 162)) #or an RGB tuple
